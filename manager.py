...
DATATYPES = SimpleNamespace(**
    {
        ...
        "INTAN"    : 1,
        "MCS"      : 2,
        "HDF5"     : 2, # Same thing as MCS for now but will have to change eventually
...
def recognize(file_path):
    ...
    if extension == ".rhd":
        ...
        return DATATYPES.HDF5
    elif extension == ".smr":
        return DATATYPES.SMR
    elif extension == ".bin":
        return DATATYPES.MED
    else:
...
def DataSource(path, import_parameters=None):
    ...
    if filetype == DATATYPES.HDF5:
        from .mcs.mcsh5_datamanager import H5DataSource
        return H5DataSource(path=path, import_parameters=import_parameters)
    if filetype == DATATYPES.MCS:
        from .mcs.mcsh5_datamanager import H5DataSource
        return H5DataSource(path=path, import_parameters=import_parameters)
    if filetype == DATATYPES.MULTIMED:
        from .multimed.bin_datamanager import BinDataSource
